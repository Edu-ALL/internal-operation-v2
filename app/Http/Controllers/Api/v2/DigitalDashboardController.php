...
class DigitalDashboardController extends Controller
{
    ...
    public function getDataLead(Request $request)
    {
        ...
        try {
            ...
            # Chart lead
            for ($i = 2; $i >= 0; $i--) {
                $dataLeadChart['target'][] = $targetTrackingLead->where('month', date('Y-m', strtotime($date->subMonth($i))))->count() > 0 ? (int)$targetTrackingLead->where('month', date('Y-m', strtotime($date->subMonth($i))))->first()->target : 0;
                $dataLeadChart['actual'][] = $targetTrackingLead->where('month', date('Y-m', strtotime($date->subMonth($i))))->count() > 0 ? (int)$targetTrackingLead->where('month', date('Y-m', strtotime($date->subMonth($i))))->first()->actual : 0;
